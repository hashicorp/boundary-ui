...
<Dropdown class="scope-picker" @enableCollisionDetection={{true}} @listPosition="bottom-left" data-test-header-scope-dropdown ...attributes as |D|>
  ...
  <D.Checkmark @icon="globe" @route="scopes.scope" @model="global" @selected={{this.scope.org.isGlobal}}>
    {{t "titles.global"}}
  </D.Checkmark>
  ...
  {{#each this.truncatedOrgsList as |org|}}
    <D.Checkmark @icon="org" @route="scopes.scope" @model={{org.id}} @selected={{and (eq this.scope.org.id org.id) (not this.scope.project)}} data-test-scope-picker-org-item>
      {{org.displayName}}
    </D.Checkmark>
  {{/each}}

  {{#if (gt this.scope.orgsList.length 5)}}
    <D.Description @text={{t "descriptions.and-more"}} data-test-scope-picker-org-and-more-text />

    <D.Interactive @route="scopes.scope.scopes" @model="global" data-test-scope-picker-org-count>{{t "descriptions.view-all-orgs" total=this.scope.orgsList.length}}</D.Interactive>
  {{/if}}
  ...
  {{#if this.scope.project}}
    ...
    {{#each this.truncatedProjectsList as |project|}}
      <D.Checkmark @icon="grid" @route="scopes.scope" @model={{project.id}} @selected={{eq this.scope.project.id project.id}} class="indentation" data-test-scope-picker-project-item>
        {{project.displayName}}
      </D.Checkmark>
    {{/each}}

    {{#if (gt this.scope.projectsList.length 5)}}
      <D.Description @text={{t "descriptions.and-more"}} class="indentation" data-test-scope-picker-project-and-more-text />

      <D.Interactive @route="scopes.scope.scopes" @model={{this.scope.org.id}} class="indentation" data-test-scope-picker-project-count>{{t "descriptions.view-all-projects" total=this.scope.projectsList.length}}</D.Interactive>
    {{/if}}
  {{/if}}
</Dropdown></template>}
