...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  {{#if (or @model.isNew form.isEditable)}}
    <Field name="password" @type="password" @value={{@model.password}} @isInvalid={{@model.errors.password}} @isRequired={{true}} disabled={{form.disabled}} autocomplete="new-password" {{on "input" (setFromEvent @model "password")}} as |F|>
