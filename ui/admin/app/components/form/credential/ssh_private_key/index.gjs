...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  {{/if}}
  <Field name="username" @value={{@model.username}} @isInvalid={{@model.errors.username}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "username")}} as |F|>
  ...
  {{#if (or @model.isNew form.isEditable)}}
    <Field0 name="private_key" @value={{@model.private_key}} @isInvalid={{@model.errors.private_key}} @isRequired={{true}} disabled={{form.disabled}} as |F|>
      <F.Label>{{t "resources.credential.form.private_key.label"}}</F.Label>
      <F.HelperText>{{t "resources.credential.form.private_key.help"}}</F.HelperText>
    ...
    <Field name="private_key_passphrase" @type="password" @value={{@model.private_key_passphrase}} @isInvalid={{@model.errors.private_key_passphrase}} disabled={{form.disabled}} autocomplete="new-password" {{on "input" (setFromEvent @model "private_key_passphrase")}} as |F|>
      <F.Label>
