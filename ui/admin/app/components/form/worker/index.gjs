...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  {{#let (relativeDatetimeLive @model.last_status_time) (formatDate @model.last_status_time format="yymmddt") as |relativeDate customFormat|}}
    <InfoField @value={{if @model.last_status_time (concat relativeDate ", " customFormat)}} readonly={{true}} as |F|>
