...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  {{!-- endpoint_url --}}
  <Field name="endpoint_url" @isRequired={{true}} @value={{@model.endpoint_url}} @isInvalid={{@model.errors.endpoint_url}} disabled={{form.disabled}} readOnly={{not @model.isNew}} {{on "input" (setFromEvent @model "endpoint_url")}} as |F|>
    <F.Label>{{t "resources.storage-bucket.form.endpoint_url.label"}}</F.Label>
    <F.HelperText>{{t "resources.storage-bucket.form.endpoint_url.help"}}</F.HelperText>
  ...
  {{!-- region --}}
  <Field name="region" @isOptional={{true}} @value={{@model.region}} @isInvalid={{@model.errors.region}} disabled={{form.disabled}} readOnly={{not @model.isNew}} {{on "input" (setFromEvent @model "region")}} as |F|>
    ...
    <Fieldset aria-labelledby={{labelId}} aria-describedby={{helpId}} class="worker-filter-generator-form-layout" @isRequired={{true}} as |F|>
      ...
      <F.Control>
        {{#if (and form.disabled (not @model.isSaving))}}
          <CodeBlock @maxHeight="300px" @language="bash" @value={{@model.worker_filter}} @hasCopyButton={{true}} data-test-worker-filter />
        {{else}}
