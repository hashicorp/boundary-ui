...
<Form @edit={{@edit}} @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  <Field0 name="filter" @value={{@model.filter_string}} @isOptional={{true}} @isInvalid={{@model.errors.filter_string}} disabled={{form.disabled}} as |F|>
    <F.Label>{{t "resources.host-set.form.filter.label"}}</F.Label>
    <F.HelperText>{{t "resources.host-set.form.filter.azure.help"}}</F.HelperText>
