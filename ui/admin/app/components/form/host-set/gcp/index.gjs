...
<template>
  ...
  >
    ...
    <InfoField @value={{t 'descriptions.provider'}} @icon='gcp-color' as |F|>
      <F.Label>{{t 'titles.provider'}}</F.Label>
      <F.HelperText>
        {{t 'resources.host-catalog.types.gcp'}}
      </F.HelperText>
    </InfoField>
    ...
    >
      <:fieldset as |F|>
        ...
        <F.HelperText>
          {{t 'resources.host-set.form.filter.gcp.help'}}
        </F.HelperText>
    ...
    <Field
      ...
      @isOptional={{true}}
      disabled={{form.disabled}}
      {{on 'input' (setFromEvent @model 'sync_interval_seconds')}}
      as |F|
    >
