...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  <Field name="filter" @value={{@model.filter_string}} @isInvalid={{@model.errors.filter_string}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "filter_string")}} as |F|>
    ...
    {{#if @model.errors.filter_string}}
      <F.Error as |E|>
        {{#each @model.errors.filter_string as |error|}}
          <E.Message data-test-error-message-filter-string>{{error.message}}</E.Message>
        {{/each}}
      </F.Error>
    {{/if}}
  </Field>
