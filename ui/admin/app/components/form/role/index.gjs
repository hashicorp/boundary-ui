...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  <Field @isOptional={{true}} @value={{@model.name}} @isInvalid={{@model.errors.name}} @type="text" name="name" readonly={{form.disabled}} {{on "input" (setFromEvent @model "name")}} as |F|>
  ...
  <Field0 @isOptional={{true}} @value={{@model.description}} @isInvalid={{@model.errors.description}} name="description" readonly={{form.disabled}} as |F|>
