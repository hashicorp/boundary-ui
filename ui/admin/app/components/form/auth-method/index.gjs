...
--}}
{{#if @model.type}}
  <this.authMethodFormComponent @model={{@model}} @edit={{@edit}} @submit={{@submit}} @cancel={{@cancel}} @removeItemByIndex={{@removeItemByIndex}} @addStringItem={{@addStringItem}} @addAccountMapItem={{@addAccountMapItem}} />
{{/if}}</template>}
