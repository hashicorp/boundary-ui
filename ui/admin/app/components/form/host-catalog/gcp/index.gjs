...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  {{#if @model.isNew}}
    <Group @name={{t "form.type.label"}} as |G|>
      ...
      {{#each @hostCatalogTypes as |hostCatalogType|}}
        <G.RadioCard @value={{hostCatalogType}} @maxWidth="20rem" @checked={{eq hostCatalogType @model.type}} {{on "input" (fn @changeType hostCatalogType)}} as |R|>
          <R.Label>{{t (concat "resources.host-catalog.types." hostCatalogType)}}</R.Label>
          <R.Description>{{t (concat "resources.host-catalog.help." hostCatalogType)}}</R.Description>
        </G.RadioCard>
      {{/each}}
    </Group>
    <Group @name={{t "form.plugin_type.label"}} @alignment="center" as |G|>
      ...
      <G.HelperText>{{concat (t "descriptions.choose-a-provider")}}</G.HelperText>
      ...
      {{#each-in @mapResourceTypeWithIcon as |pluginType icon|}}
        <G.RadioCard @value={{pluginType}} @maxWidth="20rem" @checked={{eq pluginType @model.compositeType}} {{on "input" (fn @changeType pluginType)}} as |R|>
          <R.Label>{{t (concat "resources.host-catalog.types." pluginType)}}</R.Label>
          <R.Icon @name={{icon}} />
        </G.RadioCard>
      {{/each-in}}
    </Group>
  {{else}}
    <InfoField @value={{t "resources.host-catalog.types.gcp"}} @icon="gcp-color" disabled={{true}} as |F|>
  ...
  <Field name="project_id" @value={{@model.project_id}} @isInvalid={{@model.errors.project_id}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "project_id")}} as |F|>
  ...
  <Field name="zone" @value={{@model.zone}} @isInvalid={{@model.errors.zone}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "zone")}} as |F|>
  ...
  <Field name="client_email" @value={{@model.client_email}} @isInvalid={{@model.errors.client_email}} @isOptional={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "client_email")}} as |F|>
  ...
  <Field name="target_service_account_id" @value={{@model.target_service_account_id}} @isInvalid={{@model.errors.target_service_account_id}} @isOptional={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "target_service_account_id")}} as |F|>
    <F.Label>{{t "resources.host-catalog.form.target_service_account_id.label"}}</F.Label>
    <F.HelperText>{{t "resources.host-catalog.form.target_service_account_id.help"}}
      <Inline @href={{docUrl "host-catalog.gcp"}}>
        {{t "actions.learn-more"}}
      </Inline>
    </F.HelperText>
  ...
  <Field name="private_key_id" @type="password" @value={{@model.private_key_id}} @isInvalid={{@model.errors.private_key_id}} @isOptional={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "private_key_id")}} as |F|>
  ...
  <Field0 name="private_key" @value={{@model.private_key}} @isInvalid={{@model.errors.private_key}} @isOptional={{true}} disabled={{form.disabled}} as |F|>
  ...
  {{#if (featureFlag "worker-filter")}}
    <Field0 name="worker_filter" @value={{@model.worker_filter}} @isOptional={{true}} @isInvalid={{@model.errors.worker_filter}} disabled={{form.disabled}} as |F|>
