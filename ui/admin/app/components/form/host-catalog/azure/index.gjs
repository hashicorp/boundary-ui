...
<Form @onSubmit={{@submit}} @cancel={{@cancel}} @disabled={{@model.isSaving}} @showEditToggle={{if @model.isNew false true}} as |form|>
  ...
  {{else}}
    <InfoField @value={{t "resources.host-catalog.types.azure"}} @icon="azure-color" readonly={{true}} as |F|>
  ...
  <Field name="tenant_id" @value={{@model.tenant_id}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "tenant_id")}} as |F|>
    <F.Label>{{t "form.tenant_id.label"}}</F.Label>
    <F.HelperText>
      {{t "form.tenant_id.help"}}
      <Inline @href={{docUrl "host-catalog.azure"}}>
        {{t "actions.learn-more"}}
      </Inline>
    </F.HelperText>
  </Field>

  <Field name="subscription_id" @value={{@model.subscription_id}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "subscription_id")}} as |F|>
  ...
  <Field name="client_id" @value={{@model.client_id}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "client_id")}} as |F|>
  ...
  <Field name="secret_value" @value={{@model.secret_value}} @isRequired={{true}} disabled={{form.disabled}} {{on "input" (setFromEvent @model "secret_value")}} as |F|>
  ...
  <Field1 name="disable_credential_rotation" checked={{@model.disable_credential_rotation}} disabled={{true}} as |F|>
