...
{{#let (hash canCreateHosts=(can "create model" @model collection="hosts") canCreateHostSets=(can "create model" @model collection="host-sets") canDelete=(can "delete model" @model)) as |perms|}}
  {{#if (or perms.canCreateHosts perms.canCreateHostSets perms.canDelete)}}
    <Dropdown data-test-manage-host-catalogs-dropdown as |dd|>
      ...
      {{#if perms.canCreateHosts}}
        <dd.Interactive @route="scopes.scope.host-catalogs.host-catalog.hosts.new" @model={{@model}}>
          {{t "resources.host.actions.create"}}
        </dd.Interactive>
      {{/if}}
      {{#if perms.canCreateHostSets}}
        <dd.Interactive @route="scopes.scope.host-catalogs.host-catalog.host-sets.new" @model={{@model}}>
