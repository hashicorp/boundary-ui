...
{{#let (hash canAddHosts=(can "addHosts hostSet" @model) canDelete=(can "delete model" @model)) as |perms|}}
  {{#if (or perms.canAddHosts perms.canDelete)}}
    <Dropdown data-test-manage-dropdown-host-sets as |dd|>
      ...
      {{#if perms.canAddHosts}}
        <dd.Interactive @route="scopes.scope.host-catalogs.host-catalog.host-sets.host-set.create-and-add-host">
          {{t "resources.host-set.actions.create-and-add-host"}}
        </dd.Interactive>
        <dd.Interactive @route="scopes.scope.host-catalogs.host-catalog.host-sets.host-set.add-hosts">
