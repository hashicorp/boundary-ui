...
{{#if (or @model.isOIDC @model.isLDAP)}}
  <Dropdown class="change-state" data-test-change-state as |dd|>
    ...
    <dd.ToggleButton @text={{t (concat "states." @model.state)}} @color="secondary" @icon={{if @model.isInactive "x-circle" (if @model.isPrivate "lock" "check-circle")}} />
    {{#each @model.options.state as |state|}}
      <dd.Radio checked={{eq @model.state state}} @value={{state}} {{on "change" (fn @changeState @model state)}}>
        {{t (concat "states." state)}}
      </dd.Radio>
    {{/each}}
  </Dropdown>
{{/if}}
{{#let (hash canCreateAccount=(can "create model" @model collection="accounts") canCreateManagedGroup=(can "create model" @model collection="managed-groups") canDelete=(can "delete auth-method" @model)) as |perms|}}
  {{#if (or perms.canCreateAccount perms.canCreateManagedGroup perms.canDelete)}}
    <Dropdown data-test-manage-auth-method as |dd|>
      ...
      {{#if perms.canCreateAccount}}
        <dd.Interactive @route="scopes.scope.auth-methods.auth-method.accounts.new" @model={{@model}}>
          {{t "resources.account.actions.create"}}
        </dd.Interactive>
      {{/if}}
      {{#if perms.canCreateManagedGroup}}
        {{#if (or @model.isOIDC @model.isLDAP)}}
          <dd.Interactive @route="scopes.scope.auth-methods.auth-method.managed-groups.new" @model={{@model}}>
