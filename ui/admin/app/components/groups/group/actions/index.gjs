...
{{#let (hash canAddMembers=(can "addMembers group" @model) canDelete=(can "delete model" @model)) as |perms|}}
    ...
    <Dropdown data-test-manage-group-dropdown as |dd|>
      ...
      {{#if perms.canDelete}}
        {{#if perms.canAddMembers}}
          <dd.Separator />
        {{/if}}
        <dd.Interactive @color="critical" @disabled={{@model.canSave}} {{on "click" @delete}}>
