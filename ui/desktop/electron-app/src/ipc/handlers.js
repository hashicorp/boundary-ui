...
const { getSender, removeSender } = require('./ipc-handler.js');
...
handle('removeTerminal', ({ id }) => {
  if (!terminalManager.hasTerminal(id)) {
    throw new Error('Terminal not found');
  }
  // Clean up sender associated with terminal ID
  removeSender(id);
  return terminalManager.removeTerminal(id);
});
