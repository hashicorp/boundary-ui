...
```html
<Rose::Form::HelperText @id="helper-text-{{field.id}}" @link='text link' @linkText='text goes here'>
  Helper text
...
<Preview>
  <Story name="HelperText">{{
    template: hbs`
      <Rose::Form::HelperText @error={{this.error}} @link={{this.link}} @linkText={{this.linkTxt}}>
        Helper text
      </Rose::Form::HelperText>
    `,
    context: {
