  ...
  <Story name="Advanced Message" height="12rem">{{
    ...
    <Rose::Message @title={{title}} @subtitle={{subtitle}} @icon={{icon}} as |message|>
      ...
      <message.link @route="index">
        <Rose::Icon @name="flight/chevron-left-16" />
        Go back
      </message.link>
      <message.link @route="index">
        Need help?
    ...
    context: {
      ...
      icon: select(
        'icon',
        {
          'flight/alert-circle-16': 'flight/alert-circle-16',
...
```html
<Rose::Message @title="Page not found" @subtitle="Error 404" @icon="flight/alert-circle-16" as |message|>
  <message.description>
