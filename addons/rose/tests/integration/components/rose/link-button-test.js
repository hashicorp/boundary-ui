...
module('Integration | Component | rose/link-button', function(hooks) {
  ...
  test('it renders with content', async function(assert) {
    ...
    assert.ok(find('.rose-link-button').textContent.trim(), 'Link button content');
  });

  test('it renders disabled when @disabled={{true}}', async function(assert) {
    await render(hbs`<Rose::LinkButton @disabled={{true}}/>`);
