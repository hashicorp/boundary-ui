...
module('Integration | Component | rose/header/dropdown/trigger', function (
  hooks
) {
  ...
  test('it renders with html attributes', async function (assert) {
    await render(
      hbs`<Rose::Header::Dropdown::Trigger id="trigger" class="dropdown-trigger" />`
    );
    assert.equal(find('.rose-header-dropdown-trigger').id, 'trigger');
    assert.ok(
      find('.rose-header-dropdown-trigger').className.match('dropdown-trigger')
    );
  });
