...
module('Integration | Component | rose/header/dropdown/content', function (
  hooks
) {
  ...
  test('it renders with content', async function (assert) {
    ...
    assert.equal(
      find('.rose-header-dropdown-content').textContent.trim(),
      'content'
    );
  });
  ...
  test('it renders with html attributes', async function (assert) {
    await render(
      hbs`<Rose::Header::Dropdown::Content id="dropdown-content" class="universe" />`
    );
    assert.equal(find('.rose-header-dropdown-content').id, 'dropdown-content');
    assert.ok(
      find('.rose-header-dropdown-content').className.match('universe')
    );
  });
  ...
  test('it is positioned right when @alignRight={{true}}', async function (assert) {
    ...
    await render(
      hbs`<Rose::Header::Dropdown::Content @alignRight="{{true}}"/>`
    );
