...
module('Unit | Model | target', function (hooks) {
  ...
  test('it has a `setStorageBucket` method that targets a specific POST API endpoint and serialization', async function (assert) {
    assert.expect(1);
    this.server.post(
      '/v1/targets/123abc:set-storage-bucket',
      (schema, request) => {
        const body = JSON.parse(request.requestBody);
        assert.deepEqual(body, {
    ...
    const model = store.peekRecord('target', '123abc');
    await model.setStorageBucket();
  });

  test('it has a `removeStorageBucket` method that targets a specific POST API endpoint and serialization', async function (assert) {
    assert.expect(1);
    this.server.post(
      '/v1/targets/123abc:remove-storage-bucket',
      (schema, request) => {
        const body = JSON.parse(request.requestBody);
        assert.deepEqual(body, {
          version: 1,
    ...
    const model = store.peekRecord('target', '123abc');
    await model.removeStorageBucket();
  });
});
